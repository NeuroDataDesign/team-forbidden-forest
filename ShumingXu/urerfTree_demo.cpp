    ...
    template <typename T>
        class urerfTree
        {
                    ...
                    for(unsigned int i = 0; i < siz; ++i) {
                        for (unsigned int j=0; j<=i; ++j) {
                            ...
                            if(pairMat.count(pair1) > 0){
                                #pragma omp critical
                                                               	{
                                    pairMat[pair1] += 1/nodes.depth;
                                }
                            }
                            else{
                                #pragma omp critical
                                {
                                    pairMat.insert({pair1, 1/ nodes.depth});
                                }
                            }
                        }
                    }
            ...
            inline void updateSimMatOut(std::map<int, std::map<int, int> > &simMat, std::map<std::pair<int, int>, double> &pairMat){
                for(auto nodes : leafNodes){
                    stratifiedInNodeClassIndicesUnsupervised* obsI = nodes.returnObsIndices();
                    stratifiedInNodeClassIndicesUnsupervised* nodeObsI = nodes.returnObsIndices();
                    std::vector<int> leafObs;
                    ...
                    for(unsigned int i = 0; i < siz; ++i) {
                        for (unsigned int j=0; j<=i; ++j) {
                            std::pair<int, int> pair1 = std::make_pair(leafObs[i], leafObs[j]);
                            nodeObs = nodeObsI->returnOutSampsVec();
                                        		#pragma omp critical
                            {
                                while(nodeObs.find[leafObs[i]]!=nodeObs.end() || nodeObs.find[leafObs[j]]!=nodeObs.end()){
                                    node = tree[node.parentID];
                                    nodeObsI = node.returnObsIndices();
                                    nodeObs = nodeObsI->returnOutSampsVec();
                                }
                                
                                auto it = pairMat.find(pair1);
                                if(it!=pairMat.end())
                                    pairMat[pair1] += 1/node.depth;
                                else
                                    pairMat.insert({pair1, 1/node.depth});
                            }
                        }
                    }
                }
